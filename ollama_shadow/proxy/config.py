"""Configuration management for Ollama Shadow proxy."""
...
def get_workspace_root() -> Path:
    ...
    Using CWD lets users place workspaces wherever they run `ollama-shadow start`,
...
DEFAULT_CONFIG = {
    ...
    # qwen3-coder:480b-cloud — Best for coding/exploits via Ollama Cloud.
    # Change to a local model (e.g. qwen3:32b) if not using cloud.
...
class Config:
    """Application configuration loaded from ~/.ollama-shadow/config.json."""
    ...
    def load(cls, config_path: str | Path | None = None) -> Config:
        """Load config from specified path or default ~/.ollama-shadow/config.json."""
        if config_path:
        ...
        for key in current_config:
            env_key = f"OLLAMA_SHADOW_{key.upper()}"
            if env_key in os.environ:
                ...
                if isinstance(default_val, bool):
                    ...
                    try:
                        ...
                        logger.warning("OLLAMA_SHADOW_%s env var %r is not a valid int — ignored", key.upper(), val)
                elif isinstance(default_val, float):
                    try:
                        current_config[key] = float(val)
                    except (ValueError, TypeError):
                        logger.warning("OLLAMA_SHADOW_%s env var %r is not a valid float — ignored", key.upper(), val)
                else:
