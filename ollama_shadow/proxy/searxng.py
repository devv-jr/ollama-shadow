"""SearXNG container lifecycle manager for Ollama Shadow.
...
CONTAINER_NAME = "ollama-shadow-searxng"
INTERNAL_PORT = 8080
HEALTH_TIMEOUT = 30  # seconds to wait for SearXNG to become healthy


def _config_dir() -> Path:
    """~/.ollama-shadow/searxng/ — persistent across runs."""
    d = Path.home() / ".ollama-shadow" / "searxng"
    d.mkdir(parents=True, exist_ok=True)
    return d


def _write_settings(config_dir: Path) -> None:
    """Copy containers/settings.yml to ~/.ollama-shadow/searxng/settings.yml.
    ...
    content = content.replace(
        "  # secret_key is injected at runtime by Ollama Shadow",
...
class SearXNGManager:
    """Manages the ollama-shadow-searxng Docker container lifecycle."""
    ...
    async def _start_container(self) -> bool:
        ...
        # If Ollama Shadow sandbox uses host network, we need SearXNG reachable from it too.
