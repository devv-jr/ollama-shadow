...
logger = logging.getLogger("ollama_shadow.server")
...
async def lifespan(app: FastAPI):
    ...
    cfg = get_config()
    logger.info(f"Starting Ollama Shadow Proxy on {cfg.proxy_host}:{cfg.proxy_port}")
    ...
    if engine:
        await engine.close()
    logger.info("Ollama Shadow Proxy shutdown complete")


app = FastAPI(
    title="Ollama Shadow Proxy",
    version="0.1.4",
...
def run_server() -> None:
    ...
    uvicorn.run(
        "ollama_shadow.proxy.server:app",
        host=cfg.proxy_host,
        port=cfg.proxy_port,
        log_level="critical",
        log_config=None,
