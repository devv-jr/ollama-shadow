...
description: Headless code review workflow for Ollama Shadow Docker engine (CLI-only), focused on bug discovery, security risks, regressions, and test gaps.
---
...
- Ollama Shadow engine runs in Docker + terminal tools.
...
grep -Rsn "TODO\\|FIXME\\|except Exception\\|pass$\\|eval\\|exec\\|subprocess" ollama-shadow/

# Focus on path/file handling
grep -Rsn "resolve\\|relative_to\\|os.path.join\\|open(" ollama-shadow/proxy/
