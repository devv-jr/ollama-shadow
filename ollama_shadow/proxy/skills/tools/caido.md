# Caido CLI — Complete GraphQL API Guide for Ollama Shadow

Caido is a web proxy tool (similar to Burp Suite) controlled entirely via a GraphQL API.
In the Ollama Shadow sandbox it runs on port **48080** and is managed by `caido-setup`.
...
curl -sL -X POST \
  ...
  -H "Authorization: Bearer $TOKEN" \
  -d "{
    \"query\": \"mutation { createFinding(requestId: \\\"$REQUEST_ID\\\", input: { title: \\\"IDOR - User ID Enumeration\\\", reporter: \\\"ollama-shadow\\\", description: \\\"GET /api/user?id=X leaks other users data without authorization\\\" }) { finding { id title } } }\"
  }" \
...
Caido process dead:
  kill -0 $(cat /tmp/ollama-shadow_caido.pid) && echo "alive" || echo "dead"
