...
Each `ollama-shadow start` creates a new session. Use `ollama-shadow start --session <id>` to resume.

Storage: ~/.ollama-shadow/sessions/<session_id>.json
"""
...
logger = logging.getLogger("ollama_shadow.agent.session")

SESSIONS_DIR = Path.home() / ".ollama-shadow" / "sessions"
