...
logger = logging.getLogger("ollama_shadow.agent")
...
class AgentLoop(_ValidatorMixin, _FormatterMixin,
    ...
    #   1. cd /workspace/<target> && ...  — Ollama Shadow workspace invocation pattern
    ...
    def __init__(self, ollama: OllamaClient, engine: DockerEngine) -> None:
        ...
        # True for subagents — skips OLLAMA_SHADOW_SESSION_ID env var so subagent
    ...
    async def initialize(
        ...
        import os as _os
        _session_id = _os.environ.get("OLLAMA_SHADOW_SESSION_ID") if not self._is_subagent else None
        if _session_id:
