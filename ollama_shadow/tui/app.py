"""Ollama Shadow Textual TUI Application."""
...
from ollama_shadow.proxy.config import get_workspace_root

logger = logging.getLogger("ollama_shadow.tui")


class OllamaShadowApp(App):
    """Ollama Shadow Terminal User Interface."""

    TITLE = "Ollama Shadow"
    SUB_TITLE = "Cloud-Powered Autonomous Pentesting Agent"
    ...
    async def _poll_services(self) -> None:
        ...
        if not connected:
            if proxy_reachable:
                ...
                chat.add_error_message(
                    "Cannot reach proxy server. Run `ollama-shadow proxy` "
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                if verbose:
                    ...
                    status_md = f"""## 🟢 Ollama Shadow Status Report
...
- **Docker Sandbox**: {'✅ Running' if docker_ok else '❌ Stopped'}
  - Image: `{docker.get("image", "ollama-shadow-sandbox")}`
...
"""
    ...
    async def _stream_chat_response(
        ...
        try:
            ...
            logger.error(msg)
            chat.add_error_message(
                "Cannot connect to proxy. Make sure Ollama Shadow proxy is running on "
                f"`{self.proxy_url}`"
            )
    ...
    async def _handle_slash_command(self, cmd: str) -> None:
        ...
        if cmd in ("/help", "/h"):
            chat.add_assistant_message(
                ...
                "|---------|------------|\n"
                "| `/help` | Show this help |\n"
                "| `/info` | Show Ollama Shadow information |\n"
            ...
            info_text = """## ℹ️ Ollama Shadow Information
